...
if __name__ == "__main__":
    ...
    layout = [
        [sg.TabGroup([[sg.Tab('Конвертировать в XML', layoutConvert), sg.Tab('Отправить на сервер', layoutSend)]])]
        ]
    ...
    while True:
        try:
            ...
            if event == "Конвертировать":
                if values['-USER FOLDER-'] == '':
                    sg.popup_error('Выберите файл с расширением xlsx!')
                else:
                    folder = sg.popup_get_folder("Выберите дирректорию для сохранения файла.", font=("Arial", 11))
                    if folder == '':
                        sg.popup_error('Выберите директорию для сохранения файла!')
                    else:
        ...
        #     sg.popup_error("Пожалуйста, выберите файл!", font=("Arial", 11))
            ...
            sg.popup_error("Введён неверный адрес сервера!")
